...
switch($_GET['acao_externa']){
	...
	case 'indisponibilidade_peticionamento_usuario_externo_consultar':
		...
		$strLinkFinal = $urlBase . 'md_pet_indisponibilidade_usu_ext_lista.php?acao_externa=indisponibilidade_peticionamento_usuario_externo_listar';
...
if (count($arrAcoesRemover)>0 || $_POST['hdnAnexos'] != ""  ){
		...
		foreach(array_keys($arrAcoesRemover) as $id){
			$urlBase = ConfiguracaoSEI::getInstance()->getValor('SEI','URL') . "/modulos/peticionamento/";
			$urlBaseLink = $urlBase . "md_pet_indisponibilidade_usu_ext_cadastro.php?download=1&acao_externa=indisponibilidade_peticionamento_usuario_externo_download";
        }
      }
?>
 ...
 <fieldset class="sizeFieldset infraFieldset" style="margin-top: 11px; margin-bottom: 11px;">
    ...
    <div>
        <label id="lblDescricao" class="infraLabelOpcional">
            <ul>Observação: Conforme normativo próprio, algumas indisponibilidades justificam a prorrogação automática dos prazos para a realização de atos processuais em meio eletrônico que venceriam no dia de sua ocorrência, prorrogando-os para o primeiro dia útil seguinte à resolução do problema.</ul>
        </label>
    </div>
...
$strLink = $urlBase . '/modulos/peticionamento/md_pet_indisponibilidade_usu_ext_lista.php?acao_externa=indisponibilidade_peticionamento_usuario_externo_listar&id_orgao_acesso_externo=0&id_indisponibilidade_peticionamento='.$_GET['id_indisponibilidade_peticionamento'].PaginaSEI::getInstance()->montarAncora($_GET['id_indisponibilidade_peticionamento']);
?>
