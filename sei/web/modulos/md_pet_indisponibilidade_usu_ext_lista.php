...
try {
  ...
  $strLinkPesquisar = PaginaPeticionamentoExterna::getInstance()->formatarXHTML(SessaoSEIExterna::getInstance()->assinarLink('md_pet_indisponibilidade_usu_ext_lista.php?acao_externa=indisponibilidade_peticionamento_usuario_externo_listar&id_orgao_acesso_externo=0' ) );
  ...
  if ($numRegistros > 0){
    ...
    for($i = 0;$i < $numRegistros; $i++){
      ...
      if ($bolAcaoConsultar){
        ...
        $strResultado .= '<a href="'. $urlBase . '/modulos/peticionamento/md_pet_indisponibilidade_usu_ext_cadastro.php?id_orgao_acesso_externo=0&acao_externa=indisponibilidade_peticionamento_usuario_externo_consultar&id_indisponibilidade_peticionamento='.$arrObjMdPetIndisponibilidadeDTO[$i]->getNumIdIndisponibilidade().'" tabindex="'.PaginaPeticionamentoExterna::getInstance()->getProxTabTabela().'"><img src="'.PaginaPeticionamentoExterna::getInstance()->getDiretorioImagensGlobal().'/consultar.gif" title="Consultar Indisponibilidade" alt="Consultar Indisponibilidade" class="infraImg" /></a>&nbsp;';
      }        
...
$urlForm = 'modulos/peticionamento/md_pet_indisponibilidade_usu_ext_lista.php?acao_externa=indisponibilidade_peticionamento_usuario_externo_consultar&id_orgao_acesso_externo=0';
?>
