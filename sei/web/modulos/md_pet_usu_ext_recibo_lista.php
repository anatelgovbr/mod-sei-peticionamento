	...
	if ($numRegistros > 0){
		...
		for($i = 0;$i < $numRegistros; $i++){
			...
			if($bolAcaoConsultar && $intercorrente)
			 {
				 $acao = $_GET['acao'];
				 $urlLink = 'controlador_externo.php?&acao=recibo_pet_intercorrente_usuario_externo_consultar&acao_origem='. $acao .'&acao_retorno='.$acao.'&id_md_pet_rel_recibo_protoc='. $arrObjMdPetReciboDTO[$i]->getNumIdReciboPeticionamento();
			...
			else
			 {
				...
				{
					$acao = $_GET['acao'];
					$urlLink = 'controlador_externo.php?id_md_pet_rel_recibo_protoc='. $arrObjMdPetReciboDTO[$i]->getNumIdReciboPeticionamento() .'&acao=recibo_peticionamento_usuario_externo_consultar&acao_origem='. $acao .'&acao_retorno='.$acao;
					$linkAssinado = PaginaSEIExterna::getInstance()->formatarXHTML(SessaoSEIExterna::getInstance()->assinarLink($urlLink ));
					$strResultado .= '<a href="'. $linkAssinado . '"><img src="'.PaginaSEIExterna::getInstance()->getDiretorioImagensGlobal().'/consultar.gif" title="Consultar Recibo" alt="Consultar Recibo" class="infraImg" /></a>';
				}
			 }
