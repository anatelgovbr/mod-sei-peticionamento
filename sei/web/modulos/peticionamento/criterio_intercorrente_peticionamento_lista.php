...
try {
    ...
    if ($bolAcaoCadastrar){
        $arrComandos[] = '<button type="button" accesskey="n" id="btnNovo" value="Novo Critério" onclick="location.href=\''.PaginaSEI::getInstance()->formatarXHTML(SessaoSEI::getInstance()->assinarLink('controlador.php?acao=criterio_intercorrente_peticionamento_cadastrar&acao_origem='.$_GET['acao'].'&acao_retorno='.$_GET['acao'])).'\'" class="infraButton"><span class="infraTeclaAtalho">N</span>ovo Critério</button>';
    }
    ...
    if ($numRegistros > 0){
        ...
        $strResultado .= '<th class="infraTh">Nível de Acesso dos Documentos</th>'."\n";
        ...
        for($i = 0;$i < $numRegistros; $i++){
            ...
            $indicacaoInteressado = $arrObjCriterioIntercorrentePeticionamentoDTO[$i]->getNumIdCriterioIntercorrentePeticionamento() === 'S' ? 'Próprio Usuário Externo' : 'Indicação Direta';
