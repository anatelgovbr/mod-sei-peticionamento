...
try {
  ...
  PaginaSEI::getInstance()->prepararSelecao('md_pet_acesso_externo_protocolos');
  ...
  $idsDocumentos = explode(',', $_GET['ids_documento']);
  ...
  foreach($arrObjRelProtocoloProtocoloDTO as $objRelProtocoloProtocoloDTO){
    ...
    if ($objRelProtocoloProtocoloDTO->getStrStaAssociacao()==RelProtocoloProtocoloRN::$TA_DOCUMENTO_ASSOCIADO){
      ...
      if (!in_array($objDocumentoDTO->getDblIdDocumento(), $idsDocumentos)) {
        ...
        if ($objDocumentoRN->verificarSelecaoAcessoBasico($objDocumentoDTO)) {
          ...
          if (PaginaSEI::getInstance()->isBolPaginaSelecao()) {
            $strResultado .= '<td align="center">' . PaginaSEI::getInstance()->getTrCheck($numRegistros, $objDocumentoDTO->getDblIdDocumento(), trim($objDocumentoDTO->getStrNomeSerie() . ' ' . $objDocumentoDTO->getStrNumero() . ' ' . $objDocumentoDTO->getStrNomeArvore()) . ' (' . $objDocumentoDTO->getStrProtocoloDocumentoFormatado() . ')') . '</td>';
          }
