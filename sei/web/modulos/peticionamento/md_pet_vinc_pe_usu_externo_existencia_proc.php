...
<form id="frmConcluir" method="post" action="<?= PaginaSEIExterna::getInstance()->formatarXHTML($formAction) ?>">
  ...
  $mensagem_conflito = 'Conforme listagem abaixo, não foi possível emitir a nova Procuração Eletrônica em razão de conflito com Procuração Eletrônica já existente. Para emitir a nova Procuração deve-se revogar a Procuração existente.';
  
  if($_GET['tp_procuracao'] == 'S'){
	  $conflito = '';
	  switch ($_GET['msg_conflito']) {
		  case 'poderes': $conflito = 'de poderes'; break;
		  case 'processos_e_poderes': $conflito = 'de processos e poderes'; break;
		  case 'poderes': $conflito = 'de poderes'; break;
	  }
	  $mensagem_conflito = 'Conforme listagem abaixo, não foi possível emitir a nova Procuração Eletrônica em razão de conflito '.$conflito.' com Procuração Eletrônica já existente. Para emitir a nova Procuração deve-se revisar os poderes ou revogar a(s) Procuração(ões) existente(s).';
  }
	
  ?>
    <label><?= $mensagem_conflito; ?></label>
   <?php 
   //Inicio da Lista
   ...
   if ($numRegistros > 0){
       ...
       for($i = 0;$i < $numRegistros; $i++){
           ...
           $strResultado .= '<td valign="middle">'.$arrObjMdPetVincRepresentantDTO[$i]->getStrTipoPoderesLista().'</td>';

           //Detectando Abrangência e Tratando
           $abrangencia = (in_array($arrObjMdPetVincRepresentantDTO[$i]->getStrStaAbrangencia(), ['Q', null])) ? 'Qualquer Processo em Nome do Outorgante' : 'Processos Específicos';
	       $strResultado .= '<td valign="middle">'.$abrangencia.'</td>';

           $strResultado .= '<td valign="middle">'.(!empty($arrObjMdPetVincRepresentantDTO[$i]->getDthDataLimiteValidade()) ? $arrObjMdPetVincRepresentantDTO[$i]->getDthDataLimiteValidade() : 'Indeterminado').'</td>';
           $strResultado .= '</tr>'."\n";
