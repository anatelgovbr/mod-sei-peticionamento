...
class MdPetIntercorrenteProcessoRN extends MdPetProcessoRN {
	...
	protected function incluirDocumentosApi($objProcedimentoDTO, $arrObjDocumentoAPI)
	{
		...
		foreach ($arrObjDocumentoAPI as $documentoAPI) {
			...
			$objParticipanteProcPrincDTO->setNumIdUnidade( $idUnidadeProcesso );
			...
			foreach($arrobjParticipanteProcPrinc as $objParticipanteProcPrinc){
				...
				$idsParticipantes[] = $objParticipante;
				$idsInteressados[] = $objParticipante;
				$i++;
			}
			*/
	...
	private function abrirAcessoParaAssinatura($orgaoDTO, $documentoDTO, $objParticipanteDTO)
	{
		...
		$this->objAcessoExternoDTO = $objMdPetAcessoExternoRN->cadastrar($objAcessoExternoDTO);
	}
	...
	protected function cadastrarControlado($params)
	{
		...
		$objCriterioIntercorrenteDTO = $objMdPetCriterioRN->retornarCriterioPorTipoProcesso($params['id_tipo_procedimento']);
		...
		if ($objCriterioIntercorrenteDTO->getStrSinCriterioPadrao() == 'S'
				...
				else if( $objCriterioIntercorrenteDTO != null && $objCriterioIntercorrenteDTO->getStrSinCriterioPadrao() == 'N') {
				...
				if( $idUnidadeProcesso == null || $idUnidadeProcesso == ''){
