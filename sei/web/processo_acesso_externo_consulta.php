...
try {
  ...
  if ($numProtocolos) {
    ...
    foreach ($arrObjRelProtocoloProtocoloDTO as $objRelProtocoloProtocoloDTO) {
      ...
      if ($objRelProtocoloProtocoloDTO->getStrStaAssociacao() == RelProtocoloProtocoloRN::$TA_DOCUMENTO_ASSOCIADO) {
        ...
        if ($objRelProtocoloProtocoloDTO->getStrSinAcessoExterno()=='S') {
    ...
    foreach ($arrObjRelProtocoloProtocoloDTO as $objRelProtocoloProtocoloDTO) {
      ...
      if ($objRelProtocoloProtocoloDTO->getStrStaAssociacao() == RelProtocoloProtocoloRN::$TA_DOCUMENTO_ASSOCIADO) {
        ...
        if (in_array($objDocumentoDTO->getDblIdDocumento(), $arrAcessoDocumento)) {
          if ($objDocumentoRN->verificarSelecaoGeracaoPdf($objDocumentoDTO) || $bolParaAssinatura) {
            $bolCheck = true;
          } else {
            $arrPdf[] = $objDocumentoDTO->getDblIdDocumento();
          }

          if ($bolCheck || $objDocumentoRN->verificarSelecaoGeracaoZip($objDocumentoDTO)) {
            $bolCheck = true;
          }
        }
  ...
  AuditoriaSEI::getInstance()->auditar('processo_consulta_externa', __FILE__, str_replace('&nbsp;',' ',strip_tags($strResultadoCabecalho)."\n".str_replace('[NL]',"\n",str_replace("\n",' ',str_replace("\n\n",'[NL]',strip_tags($strResultado))))));

  if ($_POST['hdnFlagGerar']=='1' || $_POST['hdnFlagGerar']=='2'){

    $strSiglaUsuarioExterno = 'Usuário Externo';
    ...
    if (isset($_GET['id_acesso_externo'])) {
      ...
      if ($objAcessoExternoDTO->getStrSiglaContato() != null) {
        $strSiglaUsuarioExterno = $objAcessoExternoDTO->getStrSiglaContato();
      }
    }else if (SessaoSEIExterna::getInstance()->getStrSiglaUsuarioExterno()!=null){
      $strSiglaUsuarioExterno = SessaoSEIExterna::getInstance()->getStrSiglaUsuarioExterno();
    }

    //Auditoria
    SessaoSEI::getInstance(false)->setStrSiglaUsuario($strSiglaUsuarioExterno);
    ...
    SeiINT::download(null, DIR_SEI_TEMP.'/'.$objAnexoDTO->getStrNome(), 'SEI-'.$objProcedimentoDTO->getStrProtocoloProcedimentoFormatado().$strExtensao, 'attachment', true);
